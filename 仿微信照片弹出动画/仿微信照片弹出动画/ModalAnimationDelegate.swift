...
class ModalAnimationDelegate: NSObject, UIViewControllerTransitioningDelegate, UIViewControllerAnimatedTransitioning {
    ...
    func presentViewAnimation(transitionContext: UIViewControllerContextTransitioning) {
        ...
        // 当前跳转的控制器
        guard let fromVc = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as? PhotoListController else {
            return;
        };
        ...
        if let img_url = selectedCell.model?.img_url {
    ...
    func dismissViewAnimation(transitionContext: UIViewControllerContextTransitioning) {
        ...
        imageView.clipsToBounds = true;
        // 获取当前显示的cell 的image
        if let img_url = dismissCell.photoModel?.img_url {
            let url = URL(string: img_url)!;
            imageView.sd_setImage(with: url);
        }
        ...
        // 取出要返回的控制器
        guard let toVC = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as? PhotoListController else {
            return;
        }
